...
exports.postChangeStatus = (req, res, next) => {
    ...
    connectDB.query(data0, (err0, result0) => {
        ...
        //     " SET  available = " + mysql.escape(result0[0].available - carwant) +
        //     " WHERE name = " + mysql.escape(cat) +
        ...
        connectDB.query(data, (err, result) => {
            if (err) throw err;
                ...
                data2 = "UPDATE category " +
                    " SET  available = " + mysql.escape(result0[0].available - req.body.want) +
                    " WHERE name = " + mysql.escape(cat) +
                    " AND type = " + mysql.escape(type);
                if (check) {
    ...
    // }
    // data2 = "UPDATE category " +
    //   " SET  available = " + mysql.escape(available)
    //   " WHERE name = " + mysql.escape(req.body.cat) +
    //     " AND type = " + mysql.escape(req.body.type);
            ...
            if (wrong == 1) {
                ...
                connectDB.query(data, (err, result) => {
                    ...
                    if (err) {
                        ...
                        res.render('admin/addcar', { msg: "Data Insert Successfuly", err: "" });
                    }
                });
            }
        })
